...
defineTool(
  ...
  async ({ chapter, section_heading }) => {
    ...
    const arc = lessons.map((l: any, i: number) => ({
      ...
      connector_hint: i === 0 ? "standalone (no connector)" : "needs connector (וכן, ולפיכך, ועוד ש, גם, אף, יתר על כן, ומכאן ש)",
    }));
  ...
  async ({ title, body }) => {
    ...
    for (const word of forbidden) {
      if (body.includes(word) || title.includes(word)) {
        issues.push(`Forbidden modern Hebrew: "${word}" — use elevated Lashon Hakodesh only`);
      }
    }
...
async function fetchLessons(chapter: string, section_heading?: string) {
  ...
  if (section_heading) q += `&section_heading=eq.${encodeURIComponent(section_heading)}`;
  q += "&limit=200";
  return await sb(q);
}
